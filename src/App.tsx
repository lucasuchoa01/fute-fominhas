...
export default function App() {
  ...
  const TabSorteio = () => {
    ...
    const doShuffle = () => {
      ...
      // Sort each team by overall desc after draw
    ...
    const removeFromTeam = (teamKey, playerId) => {
      ...
      // Add to free agents pool instead of reshuffling
    ...
    const assignFreeAgentToTeam = (agentId, teamKey) => {
      ...
      // Remove one pending slot from target team if exists
    ...
    const movePlayerBetweenTeams = (playerId, fromTeam, toTeam) => {
      ...
      const allReal = Object.values(newDrawn).flat();
      const balanced = fillPendingSlots(newDrawn, allReal);
      // Sort each team by overall desc after move
      Object.keys(balanced).forEach(tk => { balanced[tk] = sortTeam(balanced[tk]); });
      updateDrawn(balanced);
      setMovePlayer(null);
    };
    ...
    return (
      <div>
                      ...
                      >
                        ...
                        >
                          ...
                          {isAdmin && p.isPending && freeAgents.length > 0 && (
                            <button
                              onClick={() => setPendingPicker(pendingPicker?.pendingId === p.id ? null : { teamKey: k, pendingId: String(p.id) })}
                              title="Atribuir jogador disponível"
                              style={{ background: pendingPicker?.pendingId === p.id ? cfg.color + '33' : '#1a1a0a', border: `1px solid ${pendingPicker?.pendingId === p.id ? cfg.color : '#3a3a1a'}`, color: cfg.color, borderRadius: 6, padding: '2px 8px', fontSize: 11, cursor: 'pointer', fontWeight: 700 }}>
                              + Atribuir
                            </button>
                          )}
                          {isAdmin && p.isPending && pendingPicker?.pendingId === String(p.id) && (
                            <div style={{ position: 'absolute', right: 14, zIndex: 10, background: '#1a1a1a', border: `1px solid ${cfg.color}44`, borderRadius: 10, padding: 10, marginTop: 4, minWidth: 180, boxShadow: '0 4px 20px rgba(0,0,0,0.7)' }}>
                              <div style={{ fontSize: 10, color: '#555', letterSpacing: 1, marginBottom: 8 }}>ESCOLHER JOGADOR</div>
                              {freeAgents.map(fa => {
                                const faAvg = avgOverall(fa);
                                return (
                                  <button key={fa.id} onClick={() => assignFreeAgentToTeam(fa.id, k)}
                                    ...
                                    <span>{fa.name}</span>
                                    <span style={{ color: '#555', fontSize: 11 }}>overall {faAvg}</span>
                                  </button>
                                );
                              })}
        ...
        {/* Jogadores Disponíveis (removidos dos times) */}
